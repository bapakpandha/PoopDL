...
if (isset($argv[1]) && isset($argv[2])) {
    ...
    $videoPath = $tempDir . $videoId . ".mp4";
    logToFile("vid_download.log", "vid_download Argument", "Receive Argument video dengan ID: $videoId dan URL: $videoUrl");

    // $contentLength = setVideoSizeAndDuration($videoUrl, $videoId);
    // $contentLength = curl_get_file_size($videoUrl);
    // error_log(time() . "videoSize: $contentLength");

    // if ($contentLength < 15000000) {
    sleep(10);
    downloadVideo($pdo, $videoUrl, $videoPath);
    // } else {
    //     error_log(time() . "Video terlalu besar untuk diunduh");
    // }
}

function downloadVideo($pdo, $videoUrl, $videoPath)
{
    $retryCount = 0;
    $maxRetries = 3;
    $retryDelay = 14;
    ...
    do {
        ...
        curl_exec($ch);
        logToFile("vid_download.log", "function downloadVideo", "PERCOBAAN ke-{$retryCount}  -  $videoPath sedang didownload");

        if (curl_errno($ch)) {
            $error = curl_error($ch);
            logToFile("vid_download.log", "ERROR CURL function downloadVideo for $videoPath", "Download Error: " . $error);
            ...
            if (strpos($error, 'Could not resolve host') !== false) {
                ...
                if ($retryCount < $maxRetries) {
                    ...
                    logToFile("vid_download.log", "ERROR CURL function downloadVideo for $videoPath", "Failed to download after $maxRetries attempts due to 'Could not resolve host'");
                    exit;
                }
            } else {
                break;
    ...
    logToFile("vid_download.log", "Success function downloadVideo", "$videoPath selesai didownload");
...
function generateThumbnailAsync($pdo, $videoPath)
{
    ...
    if (!file_exists($ffprobePath)) {
        logToFile("vid_download.log", "ERROR generateThumbnailAsync", "ffmpeg tidak ditemukan di path: " . $ffprobePath);
    ...
    try {
        ...
        logToFile("vid_download.log", "ERROR calculateIntervalAndScale", "Error: " . $e->getMessage());
    ...
    shell_exec($command);
    logToFile("vid_download.log", "function generateThumbnailAsync", "Mengeksekusi command: $command");
...
function getTotalFrames($videoPath)
{
    ...
    if (!file_exists($videoPath)) {
        logToFile("vid_download.log", "ERROR getTotalFrames", "File video tidak ditemukan: $videoPath");
    ...
    if (!file_exists($ffprobePath)) {
        logToFile("vid_download.log", "ERROR getTotalFrames", "ffprobe tidak ditemukan di path: " . $ffprobePath);
    ...
    $outputs = [];
    $returnVar = 0;
    logToFile("vid_download.log", "function getTotalFrames for: $videoPath", "Mengeksekusi command: $command");
    exec($command, $outputs, $returnVar);

    if ($returnVar !== 0) {
        logToFile("vid_download.log", "ERROR getTotalFrames for: $videoPath", "Gagal mendapatkan total frames. Output: " . implode("\n", $outputs));
    }

    $output = intval($outputs[0]);

    if ($output === null || trim($output) === '' || ($output === 0) || (((int)trim($output)) < 600)) {    
        logToFile("vid_download.log", "ERROR getTotalFrames for: $videoPath", "Gagal mendapatkan jumlah frame dari video: $videoPath. Hasil output: $output");
	$outputs = null;
        $command = sprintf(
        ...
        if ($returnVar !== 0) {
            logToFile("vid_download.log", "ERROR getTotalFrames for: $videoPath", "Gagal mendapatkan frame video. Output: " . implode("\n", $outputs));
        }

        $jsonOutput = implode("\n", $outputs);

        $metadata = json_decode($jsonOutput, true);
        $output = $metadata['streams'][0]['nb_frames'] ?? null;
    }

    logToFile("vid_download.log", "function getTotalFrames for: $videoPath", "Total Frame: $output");
...
function getVideoResolution($videoPath)
{
    ...
    $output = [];
    $returnVar = 0;
    logToFile("vid_download.log", "function getVideoResolution for: $videoPath", "Mengeksekusi command: $command");
    exec($command, $output, $returnVar);

    if ($returnVar !== 0) {
        logToFile("vid_download.log", "ERROR getVideoResolution for: $videoPath", "Gagal mendapatkan resolusi video. Output: " . implode("\n", $output));
    ...
    if (count($resolution) !== 2) {
        logToFile("vid_download.log", "ERROR getVideoResolution for: $videoPath", "Output resolusi tidak valid: $output");
        // fallback
...
function calculateIntervalAndScale($videoPath)
{
    ...
    try {
        ...
        logToFile("vid_download.log", "function calculateIntervalAndScale for: $videoPath", "Total Frame: $totalFrames");
    } catch (Exception $e) {
        logToFile("vid_download.log", "ERROR calculateIntervalAndScale for: $videoPath", "Error: " . $e->getMessage());
        // echo "Error: " . $e->getMessage();
    }

    try {
        $resolution = getVideoResolution($videoPath);
        logToFile("vid_download.log", "function calculateIntervalAndScale for: $videoPath", "Resolution: {$resolution['width']}x{$resolution['height']}");
    ...
    if ($totalFrames <= 0) {
        logToFile("vid_download.log", "ERROR calculateIntervalAndScale for: $videoPath", "Total frame video tidak valid.");
...
function setVideoSizeAndDurationFromDownloaded($pdo, $videoPath, $videoId)
{
    ...
    $output = [];
    $returnVar = 0;
    logToFile("vid_download.log", "function setVideoSizeAndDurationFromDownloaded for: $videoPath", "Mengeksekusi command: $command");
    exec($command, $output, $returnVar);

    if ($returnVar !== 0) {
        logToFile("vid_download.log", "ERROR setVideoSizeAndDurationFromDownloaded for: $videoPath", "Gagal mendapatkan durasi video. Output: " . implode("\n", $output));
        throw new Exception("Gagal mendapatkan durasi video. Output: " . implode("\n", $output));
    }
    ...
    logToFile("vid_download.log", "function setVideoSizeAndDurationFromDownloaded for: $videoPath", "Durasi Video: " . gmdate("H:i:s", $duration) . "\n" . "Ukuran Video: " . $contentLength . " bytes");
...
function getVideoMetadata($videoPath)
{
    ...
    $output = [];
    $returnVar = 0;
    logToFile("vid_download.log", "function getVideoMetadata for: $videoPath", "Mengeksekusi command: $command");
    exec($command, $output, $returnVar);

    if ($returnVar !== 0) {
        logToFile("vid_download.log", "ERROR getVideoMetadata for: $videoPath", "Gagal mendapatkan metadata video. Output: " . implode("\n", $output));
